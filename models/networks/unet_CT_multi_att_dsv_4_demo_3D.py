...
class unet_CT_multi_att_dsv_4_demo_3D(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
        ...
        super(unet_CT_multi_att_dsv_4_demo_3D, self).__init__()
        ...
        self.conv2 = UnetConv3(filters[0], filters[1], self.is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1), init_stride=2)
        self.conv3 = UnetConv3(filters[1], filters[2], self.is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1), init_stride=2)
        self.conv4 = UnetConv3(filters[2], filters[3], self.is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1), init_stride=2)
        self.center = UnetConv3(filters[3], filters[4], self.is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1), init_stride=2)
        ...
        # deep supervision
        self.dsv5 = UnetDsv3(in_size=filters[4], out_size=n_classes, scale_factor=16)
    ...
    def forward(self, inputs):
        ...
        g_conv4, att4, fmap4 = self.attentionblock4(conv4, gating)
        up4 = self.up_concat4(g_conv4, center)
        g_conv3, att3, fmap3 = self.attentionblock3(conv3, up4)
        up3 = self.up_concat3(g_conv3, up4)
        g_conv2, att2, fmap2 = self.attentionblock2(conv2, up3)
        up2 = self.up_concat2(g_conv2, up3)
        g_conv1, att1, fmap1 = self.attentionblock1(conv1, up2)
        ...
        dsv2 = self.dsv2(up2)
        dsv1 = self.dsv1(up1)
        final = self.final(torch.cat([dsv1,dsv2,dsv3,dsv4,dsv5], dim=1))
...
class MultiAttentionBlock(nn.Module):
    ...
    def forward(self, input, gating_signal):
        gate_1, attention_1, fmap_att1 = self.gate_block_1(input, gating_signal)
        gate_2, attention_2, fmap_att2 = self.gate_block_2(input, gating_signal)

        return self.combine_gates(torch.cat([gate_1, gate_2], 1)), \
               torch.cat([attention_1, attention_2], 1), torch.cat([fmap_att1, fmap_att2], 1)
